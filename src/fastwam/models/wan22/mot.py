...
class MoT(nn.Module):
    ...
    def forward_joint_batched_action_core(
        self,
        video_tokens: torch.Tensor,
        action_tokens: torch.Tensor,
        video_freqs: torch.Tensor,
        ...
        """Run one shared video trajectory with a batch of action trajectories.

        Video queries never attend to action tokens in FastWAM's attention mask,
        so the video branch can remain batch size one. Action queries reuse the
        shared video's per-layer K/V after expanding those views to the action
        batch. This avoids physically duplicating the expensive video branch.
        """
        video_expert = self.mixtures["video"]
        action_expert = self.mixtures["action"]
        video_seq_len = int(video_tokens.shape[1])
        action_batch = int(action_tokens.shape[0])
        ...
        for layer_idx in range(self.num_layers):
            ...
            mixed_video = flash_attention(
                q=q_video,
                k=k_video,
                v=v_video,
                num_heads=self.num_heads,
                ctx_mask=video_attention_mask.to(device=q_video.device),
            )
            mixed_action = flash_attention(
                q=q_action,
                k=torch.cat(
                    [k_video.expand(action_batch, *k_video.shape[1:]), k_action],
                    dim=1,
                ),
                v=torch.cat(
                    [v_video.expand(action_batch, *v_video.shape[1:]), v_action],
                    dim=1,
                ),
                num_heads=self.num_heads,
                ctx_mask=action_attention_mask.to(device=q_action.device),
            )
            x_video = self._apply_expert_post_block_tensor(
                block=video_block,
                residual_x=residual_video,
            ...
            x_action = self._apply_expert_post_block_tensor(
                block=action_block,
                residual_x=residual_action,
