...
class FastWAM(torch.nn.Module):
    ...
    def _joint_denoise_core_batched_action(
        self,
        latents_video: torch.Tensor,
        latents_action: torch.Tensor,
        timestep_video: torch.Tensor,
        ...
        """Denoise one video and K actions without replicating the video branch."""
        (
            video_tokens,
            t_video,
            t_mod_video,
            prepared_video_context,
            prepared_video_context_mask,
            freqs_video,
            f_video,
            h_video,
            w_video,
        ...
        video_tokens, action_tokens = self.mot.forward_joint_batched_action_core(
            video_tokens=video_tokens,
            action_tokens=action_tokens,
            video_freqs=freqs_video,
            action_freqs=freqs_action,
            video_t_mod=t_mod_video,
            action_t_mod=t_mod_action,
            video_context=prepared_video_context,
            video_context_mask=prepared_video_context_mask,
            action_context=prepared_action_context,
            action_context_mask=prepared_action_context_mask,
            attention_mask=attention_mask,
        )
        return (
            self.video_expert.post(video_tokens, t_video, f_video, h_video, w_video),
    ...
    def infer_joint(
        ...
        action_denoise_trace: bool = False,
    ) -> dict[str, Any]:
        self.eval()
        ...
        latents_action = torch.randn(
            (num_action_candidates, action_horizon, self.action_expert.action_dim),
        ...
        if compile_action_infer:
            ...
            if num_action_candidates > 1:
                if not hasattr(self, "_joint_denoise_core_batched_action_compiled_inference"):
                    self._joint_denoise_core_batched_action_compiled_inference = torch.compile(
                        self._joint_denoise_core_batched_action,
                        mode="reduce-overhead",
                        fullgraph=True,
                    )
                joint_denoise_core = self._joint_denoise_core_batched_action_compiled_inference
        else:
            joint_denoise_core = (
                self._joint_denoise_core_batched_action
                if num_action_candidates > 1
                else self._joint_denoise_core
            )

        action_context = context
        action_context_mask = context_mask
        if num_action_candidates > 1:
            action_context = context.expand(num_action_candidates, -1, -1)
            action_context_mask = context_mask.expand(num_action_candidates, -1)
        ...
        action_denoise_diagnostics = []
        traced_clean_action_estimates = []
        for step_index, (step_t_video, step_delta_video, step_t_action, step_delta_action) in enumerate(zip(
            ...
            if action_denoise_trace:
                ...
                action_denoise_diagnostics.append(
                    {
                        "step": int(step_index),
                        "timestep": float(step_t_action.float().item()),
                        "sigma": float(
                            (step_t_action.float()
                             / float(self.infer_action_scheduler.num_train_timesteps)).item()
                        ),
                        "delta_sigma": float(step_delta_action.float().item()),
                        "action_shape": list(clean_action_estimate.shape),
                    }
                )
                traced_clean_action_estimates.append(clean_action_estimate.detach().cpu())
        ...
        if test_action_with_infer_action:
            if not torch.allclose(action_out[0], action_only_out, atol=1e-2, rtol=1e-2):
                max_abs_diff = (action_out[0] - action_only_out).abs().max().item()
                logger.warning(
        ...
        if action_denoise_trace:
            result["action_denoise_trace"] = action_denoise_diagnostics
            result["action_denoise_estimates"] = traced_clean_action_estimates
            result["action_denoise_final_action"] = action_out
            result["action_denoise_final_clean_estimate"] = traced_clean_action_estimates[-1]
            result["action_denoise_action_shape"] = list(action_out.shape)
            result["action_denoise_num_steps"] = len(action_denoise_diagnostics)
        return result
    @torch.no_grad()
    def infer_action(
        self,
        ...
        # The current-frame visual cache is identical for every proposal.  Keep
        # one visual forward, then batch only the independently sampled action
        # trajectories through the action expert.
        if num_action_candidates > 1:
        ...
        previous_clean_action_estimate = None
        initial_action_noise = (
            latents_action.float().detach().cpu().clone() if action_denoise_trace else None
        )
        traced_samples = []
        traced_clean_action_estimates = []
        ...
        for step_index, (step_t_action, step_delta_action) in enumerate(
            ...
            if action_denoise_trace:
                sample = latents_action.float()
                velocity = pred_action.float()
                flow_update = velocity * step_delta_action.float()
                clean_action_estimate = self.infer_action_scheduler.estimate_clean_sample(
                    model_output=velocity,
                    sample=sample,
                    timestep=step_t_action,
                )
                sigma = step_t_action.float() / float(
                    self.infer_action_scheduler.num_train_timesteps
                )
                sample_flat = sample.flatten(start_dim=1)
                velocity_flat = velocity.flatten(start_dim=1)
                clean_action_flat = clean_action_estimate.flatten(start_dim=1)
                initial_noise_flat = initial_action_noise.to(device=sample.device).flatten(start_dim=1)
                initial_noise_norm = initial_noise_flat.norm(dim=-1).clamp_min(1e-8)
                trace = {
                    "step": int(step_index),
                    "timestep": float(step_t_action.float().item()),
                    "sigma": float(sigma.item()),
                    "delta_sigma": float(step_delta_action.float().item()),
                    "velocity_rms": float(velocity.square().mean().sqrt().item()),
                    "velocity_l2_mean": float(velocity.norm(dim=-1).mean().item()),
                    "velocity_max_abs": float(velocity.abs().max().item()),
                    "velocity_rms_per_dim": velocity.square().mean(dim=(0, 1)).sqrt().tolist(),
                    "flow_update_rms": float(flow_update.square().mean().sqrt().item()),
                    "flow_update_l2_mean": float(flow_update.norm(dim=-1).mean().item()),
                    "flow_update_rms_per_dim": flow_update.square().mean(dim=(0, 1)).sqrt().tolist(),
                    "sample_rms": float(sample.square().mean().sqrt().item()),
                    "clean_action_rms": float(
                        clean_action_estimate.square().mean().sqrt().item()
                    ),
                    "sample_cosine_to_initial_noise": float(
                        F.cosine_similarity(sample_flat, initial_noise_flat, dim=-1).mean().item()
                    ),
                    "sample_relative_l2_to_initial_noise": float(
                        ((sample_flat - initial_noise_flat).norm(dim=-1) / initial_noise_norm)
                        .mean()
                        .item()
                    ),
                    "clean_action_cosine_to_initial_noise": float(
                        F.cosine_similarity(
                            clean_action_flat, initial_noise_flat, dim=-1
                        ).mean().item()
                    ),
                    "clean_action_relative_l2_to_initial_noise": float(
                        (
                            (clean_action_flat - initial_noise_flat).norm(dim=-1)
                            / initial_noise_norm
                        )
                        .mean()
                        .item()
                    ),
                    "velocity_cosine_to_previous": None,
                    "velocity_relative_l2_to_previous": None,
                    "clean_action_cosine_to_previous": None,
                    "clean_action_relative_l2_to_previous": None,
                }
                if previous_velocity is not None:
                    previous_velocity_flat = previous_velocity.flatten(start_dim=1)
                    previous_clean_action_flat = previous_clean_action_estimate.flatten(
                        start_dim=1
                    )
                    trace["velocity_cosine_to_previous"] = float(
                        F.cosine_similarity(
                            velocity_flat,
                            previous_velocity_flat,
                            dim=-1,
                        ).mean().item()
                    )
                    trace["velocity_relative_l2_to_previous"] = float(
                        (
                            (velocity_flat - previous_velocity_flat).norm(dim=-1)
                            / previous_velocity_flat.norm(dim=-1).clamp_min(1e-8)
                        )
                        .mean()
                        .item()
                    )
                    trace["clean_action_cosine_to_previous"] = float(
                        F.cosine_similarity(
                            clean_action_flat,
                            previous_clean_action_flat,
                            dim=-1,
                        ).mean().item()
                    )
                    trace["clean_action_relative_l2_to_previous"] = float(
                        (
                            (clean_action_flat - previous_clean_action_flat).norm(dim=-1)
                            / previous_clean_action_flat.norm(dim=-1).clamp_min(1e-8)
                        )
                        .mean()
                        .item()
                    )
                action_denoise_diagnostics.append(trace)
                traced_samples.append(sample.detach().cpu())
                traced_clean_action_estimates.append(clean_action_estimate.detach().cpu())
                previous_velocity = velocity.detach()
                previous_clean_action_estimate = clean_action_estimate.detach()

            if guidance_enabled and step_index in guidance_after_steps:
                # The predictor was trained on executable (clean) actions, not
                # on the noisy flow state. For v=epsilon-a0 and
                # a_sigma=(1-sigma)*a0+sigma*epsilon, a0=a_sigma-sigma*v.
                sigma = (
                    step_t_action.float()
        ...
        if action_denoise_trace:
            final_action = latents_action.float().detach().cpu()
            final_action_flat = final_action.flatten(start_dim=1)
            final_action_norm = final_action_flat.norm(dim=-1).clamp_min(1e-8)
            initial_noise_flat = initial_action_noise.flatten(start_dim=1)
            initial_to_final_cosine = float(
                F.cosine_similarity(initial_noise_flat, final_action_flat, dim=-1)
                .mean()
                .item()
            )
            initial_to_final_relative_l2 = float(
                (
                    (initial_noise_flat - final_action_flat).norm(dim=-1)
                    / final_action_norm
                )
                .mean()
                .item()
            )
            for trace, sample, clean_action_estimate in zip(
                action_denoise_diagnostics,
                traced_samples,
                traced_clean_action_estimates,
            ):
                sample_flat = sample.flatten(start_dim=1)
                clean_action_flat = clean_action_estimate.flatten(start_dim=1)
                trace["sample_cosine_to_final_action"] = float(
                    F.cosine_similarity(sample_flat, final_action_flat, dim=-1)
                    .mean()
                    .item()
                )
                trace["sample_relative_l2_to_final_action"] = float(
                    (
                        (sample_flat - final_action_flat).norm(dim=-1)
                        / final_action_norm
                    )
                    .mean()
                    .item()
                )
                trace["clean_action_cosine_to_final_action"] = float(
                    F.cosine_similarity(clean_action_flat, final_action_flat, dim=-1)
                    .mean()
                    .item()
                )
                trace["clean_action_relative_l2_to_final_action"] = float(
                    (
                        (clean_action_flat - final_action_flat).norm(dim=-1)
                        / final_action_norm
                    )
                    .mean()
                    .item()
                )
                trace["initial_noise_cosine_to_final_action"] = initial_to_final_cosine
                trace["initial_noise_relative_l2_to_final_action"] = (
                    initial_to_final_relative_l2
                )
        ...
        if action_denoise_trace:
            ...
            result["action_denoise_final_clean_estimate"] = traced_clean_action_estimates[-1]
            result["action_denoise_action_shape"] = list(action_out.shape)
            result["action_denoise_num_steps"] = total_action_steps
        return result
